...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun resignUp(){ // 회원가입 진행 함수 (1번 이상 비정상적으로 입력했을 경우)
        ...
        // 잘못 입력하여 다시 학번, 비밀번호를 입력한 후 그 값이 올바른지 확인하는 함수 호출 (resignCheckUp)
            // xml에서 editText 입력이 signUpStudentNumberEt, signUpStudentNumberEtEr 두 가지로 나누어져 있기 때문에
            // 1번 이상 비정상적으로 입력했을 경우 signUpStudentNumberEtEr에 입력
